...
class VGGNet():
    ...
    def net(self, input, args, class_dim=1000):
        ...
        conv1 = self.conv_block(input, 64, nums[0], name="conv1_", data_format=args.data_format)
        conv2 = self.conv_block(conv1, 128, nums[1], name="conv2_", data_format=args.data_format)
        conv3 = self.conv_block(conv2, 256, nums[2], name="conv3_", data_format=args.data_format)
        conv4 = self.conv_block(conv3, 512, nums[3], name="conv4_", data_format=args.data_format)
        conv5 = self.conv_block(conv4, 512, nums[4], name="conv5_", data_format=args.data_format)
    ...
    def conv_block(self, input, num_filter, groups, name=None, data_format='NCHW'):
        ...
        return fluid.layers.pool2d(
            input=conv, pool_size=2, pool_type='max', pool_stride=2, data_format=data_format)
